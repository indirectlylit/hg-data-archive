...
@click.option('--out', type=click.Path(exists=False, dir_okay=False, readable=True), default="")
@click.option('--headers', type=click.STRING, default="")
def cli(out, headers):

    app.outputFile = out

    if not out and not headers:
        click.echo("Must specify either an output file or headers.")
        exit()


    if headers:
        if not csv.Sniffer().has_header(headers):
            click.echo("'{}' does not appear to be a proper header line.\nRun with --help for assistance.".format(app.headers), err=True)
            exit()
        app.headers = re.split('\W+', headers)
        ...
        with open(app.outputFile) as f:
            ...
            if not csv.Sniffer().has_header(contents):
                ...
                exit()
            app.headers = re.split('\W+', contents.splitlines()[0].strip())
    ...
    updatePrompt()
    if not out:
        fileInfo = "Output file not specified. Will write to [uid].tsv"
    else:
